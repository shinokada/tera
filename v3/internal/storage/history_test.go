...
func TestSearchHistoryStore(t *testing.T) {
	...
	t.Run("AddSearchItem_Duplicate_MovesToTop", func(t *testing.T) {
		// Add more items
		_ = store.AddSearchItem(ctx, "name", "BBC")
		_ = store.AddSearchItem(ctx, "language", "english")
	...
	t.Run("AddSearchItem_MaxSize_Trim", func(t *testing.T) {
		// Clean slate
		_ = store.ClearSearchHistory(ctx)

		// Set small max size
		_ = store.UpdateHistorySize(ctx, 3)

		// Add 5 items
		_ = store.AddSearchItem(ctx, "tag", "item1")
		_ = store.AddSearchItem(ctx, "tag", "item2")
		_ = store.AddSearchItem(ctx, "tag", "item3")
		_ = store.AddSearchItem(ctx, "tag", "item4")
		_ = store.AddSearchItem(ctx, "tag", "item5")
	...
	t.Run("AddLuckyQuery_Duplicate_MovesToTop", func(t *testing.T) {
		// Add more queries
		_ = store.AddLuckyQuery(ctx, "jazz")
		_ = store.AddLuckyQuery(ctx, "classical")

		// Add duplicate
		_ = store.AddLuckyQuery(ctx, "meditation")
	...
	t.Run("UpdateHistorySize_Decrease_TrimsItems", func(t *testing.T) {
		// Add some items
		_ = store.ClearSearchHistory(ctx)
		_ = store.UpdateHistorySize(ctx, 10)

		for i := 1; i <= 10; i++ {
			_ = store.AddSearchItem(ctx, "tag", string(rune('a'+i-1)))
		}
	...
	t.Run("ClearSearchHistory", func(t *testing.T) {
		// Add items
		_ = store.AddSearchItem(ctx, "tag", "test1")
		_ = store.AddLuckyQuery(ctx, "test2")
	...
	t.Run("LoadSearchHistory_FileNotExist", func(t *testing.T) {
		...
		defer func() { _ = os.RemoveAll(newTmpDir) }()
